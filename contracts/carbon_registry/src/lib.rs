...
impl CarbonRegistryContract {
    ...
    /// - `oracle_address`: The address authorized to submit monitoring data and update project status
    /// - `verifiers`: Vector of addresses authorized to verify and reject projects
    pub fn initialize(
        env: Env,
        admin: Address,
    ...
    ///
    /// # Parameters
    /// - `oracle_address`: The authorized oracle address
    /// - `project_id`: The project identifier
    /// - `amount`: The amount of credits issued
    ///
    /// # Errors
    /// - [`CarbonError::UnauthorizedOracle`] if caller is not the registered oracle
    /// - [`CarbonError::ProjectNotFound`] if project does not exist
